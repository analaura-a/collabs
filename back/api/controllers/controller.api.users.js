...
const getUserCollaborationStats = async (req, res) => {

    const { userId } = req.params;

    try {
        const stats = await service.getUserCollaborationStats(userId);
        ...
        res.status(500).json({ message: `Error al obtener estadísticas de colaboración: ${error.message}` });
    }
};
